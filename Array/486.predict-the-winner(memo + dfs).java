...
class Solution {
    ...
    public boolean PredictTheWinner(int[] nums) {
        ...
        return optimunChoice(nums, 0, length - 1, cache) >= 0;
    }

    private int optimunChoice(int[] nums, int start, int end, int[][] cache){
        if(end == start) return nums[start];
        if(cache[start][end] == 0){
            int max = 0;
            max = Math.max(nums[start] - optimunChoice(nums, start + 1, end, cache), nums[end] - optimunChoice(nums, start, end - 1, cache));
            cache[start][end] = max;
        }
